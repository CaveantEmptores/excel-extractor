# file: R/plot_sbar_line.R

#' Combo: stacked vertical bars + line (sec-axis supported)
#'
#' @inheritParams plot_cbar_line
#' @export
#' @importFrom dplyr filter mutate n_distinct
#' @importFrom ggplot2 ggplot aes geom_col geom_line geom_point labs theme scale_y_continuous scale_x_discrete element_text position_stack
#' @importFrom ggplot2 expansion sec_axis
#' @importFrom stringr str_wrap
#' @importFrom scales label_percent label_number
#' @importFrom rlang .data
plot_sbar_line <- function(df, width = 8, height = 5, base_size = 12,
                           out_file = NULL, th = brand_theme_defaults,
                           palette_fill = NULL, palette_line = NULL,
                           show_bar_labels = "auto",
                           bar_label_format = scales::label_percent(accuracy = 1),
  ...
  lines <- df %>%
    ...
    dplyr::mutate(X_cat = factor(.data$X, levels = unique(.data$X)))
  if (nrow(bars) == 0 || nrow(lines) == 0) rlang::abort("plot_sbar_line() needs \u22651 bar and \u22651 line series.", call = NULL)
  ...
  if (is.null(palette_fill) || is.null(palette_line)) {
    pal <- auto_palettes_for_chart(df, "sbar_line")
  ...
  p <- ggplot2::ggplot() +
    ggplot2::geom_col(data = bars, ggplot2::aes(x = .data$X_cat, y = .data$Y, fill = .data$Series),
                      position = "stack", width = th$bar_width) +
  ...
  # optional labels inside stacks (percent threshold like plot_cbar_stack)
  labs_vec <- bars %>%
    dplyr::mutate(.lbl = make_stacked_labels(dplyr::cur_data(), label_threshold = 0.07,
                                             label_format = bar_label_format)) %>%
    dplyr::pull(.data$.lbl)
  if (!identical(show_bar_labels, "off") && any(nzchar(labs_vec))) {
